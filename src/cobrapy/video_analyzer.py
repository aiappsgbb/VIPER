...
class VideoAnalyzer:
    ...
    def analyze_video(
        ...
        **kwargs,
        ...
        results_list_output_path_as = os.path.join(
            self.manifest.processing_params.output_directory,
            f"_action_summary_{analysis_config.name}.json",
        )
        ...
        try:
            ...
            print(results_list)
            raise ValueError("bad data generated by model")
        ...
        try:
            ...
            final_video_analysis = []
            for item in results_list:
                for elem in item:
                    final_video_analysis.append(elem)

            with open(results_list_output_path, "w", encoding="utf-8") as f:
                f.write(json.dumps(final_video_analysis, indent=4))
        except:
            print(results_list)
            results_list_output_path = os.path.join(
                self.manifest.processing_params.output_directory,
