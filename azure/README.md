...
This directory contains infrastructure-as-code assets for deploying the CobraPy backend and Cobra UI frontend to [Azure Container Apps](https://learn.microsoft.com/azure/container-apps/).
...
- **`containerapps.bicep`** provisions:
  ...
  - Ingress for the frontend (public) and an internal-only endpoint for the backend. The template automatically injects a `COBRAPY_BASE_URL` environment variable into the frontend so it can call the backend over the Container Apps internal domain.
...
1. Run `scripts/Setup-CobraContainers.ps1` to install CobraPy locally, build the Docker images, and verify the services communicate through Docker.
2. Deploy to Azure by executing `scripts/Deploy-CobraToAzure.ps1`. At a minimum provide your Azure subscription, target resource group, and region:

   ```powershell
   ./scripts/Deploy-CobraToAzure.ps1 \ 
       -SubscriptionId "00000000-0000-0000-0000-000000000000" \ 
       -ResourceGroupName "cobra-prod" \ 
       -Location "eastus"
   ```
...
To override the URL that the frontend uses to reach the backend in Azure, set `COBRAPY_BASE_URL` in your `.env` before running the deployment script. Otherwise the template will generate an internal URL based on the Container Apps environment domain.
